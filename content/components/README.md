...
2. `python3 content/components/make_seed.py` — перегенерирует `cc_components_seed.sql`,
   скрипт упадёт с понятной ошибкой, если `type/name` не найдены в каталоге (не даёт
   выдумать несуществующий компонент).
3. Закоммитить `selection.json` + `cc_components_seed.sql`.
4. На сервере: `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f content/components/cc_components_seed.sql`.
