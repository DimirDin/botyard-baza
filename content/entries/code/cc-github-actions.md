...
Это тот же headless-режим (см. отдельную статью), но с готовой обвязкой под конкретную платформу —
не нужно самому писать workflow-скрипт, парсящий события GitHub и вызывающий `claude -p` вручную,
готовый action уже умеет читать контекст PR/issue, публиковать ответ обратно комментарием и открывать
PR по правилам репозитория. Это разница между «headless-режим как универсальный примитив» и «headless-
режим, подключённый конкретно к событиям GitHub одной строкой конфигурации».
