...
- **Миграция с фиксированного budget_tokens требует повторного A/B** — старая калибровка бюджета под
  конкретный профиль трафика не переносится напрямую в уровень effort один в один, стоит заново
  сверить качество и стоимость на реальных запросах, а не полагаться, что `high` эквивалентен
  прежнему точно подобранному числу токенов.
