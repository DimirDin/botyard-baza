...
- **Голый эмбеддинг-поиск слабее contextual retrieval** — чистый vector search без гибрида с BM25
  чаще промахивается.
- **Смена модели эмбеддингов требует полного переиндексирования** — векторы разных моделей (или даже
  разных версий одной модели) не сопоставимы напрямую в одном индексе, апгрейд модели эмбеддингов —
  это перегенерация всех векторов базы заново, а не точечное обновление.
