...
- **Типизированные события — не опциональная деталь, а обязательный протокол** — клиент, читающий
  поток как сырой текст без разбора типов событий, рано или поздно ломается на первом же ответе с
  несколькими content-блоками (например, thinking + text) или с вызовом тула внутри, где порядок и
  границы блоков важны для корректной сборки итогового сообщения.
