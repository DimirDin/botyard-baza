...
Практическая ценность выше там, где размер входных данных заранее неизвестен — пользователь
прикладывает файл произвольного размера, история чата растёт непредсказуемо долго, или контент
собирается динамически из нескольких источников. Вместо того чтобы отправлять запрос и обрабатывать
ошибку превышения лимита постфактум, можно заранее проверить и либо обрезать/сжать контент, либо
предупредить пользователя — что даёт лучший UX, чем ошибка после нескольких секунд ожидания.
